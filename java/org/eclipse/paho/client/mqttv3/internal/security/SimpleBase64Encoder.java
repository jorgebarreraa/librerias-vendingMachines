...
public class SimpleBase64Encoder {
    ...
    public static String encode(byte[] bArr) {
        ...
        int i = 0;
        while (length >= 3) {
            stringBuffer.append(to64(((bArr[i] & UByte.MAX_VALUE) << 16) | ((bArr[i + 1] & UByte.MAX_VALUE) << 8) | (bArr[i + 2] & UByte.MAX_VALUE), 4));
            i += 3;
            length -= 3;
        }
        if (length == 2) {
            stringBuffer.append(to64(((bArr[i] & UByte.MAX_VALUE) << 8) | (bArr[i + 1] & UByte.MAX_VALUE), 3));
        }
        if (length == 1) {
            stringBuffer.append(to64(bArr[i] & UByte.MAX_VALUE, 2));
        }
